											...
											<div class="featured-job-card-title">
												<h3>
													<a class="res-jobtitle"
														href="${basePath }job/tutor/${item.id }"
														target="_blank"> ${item.title}</a>
												</h3>
											</div>
											<div class="featured-job-card-body">
												<div class="text-primary">类别：家教兼职</div>
							...
							<nav>
								...
								<c:choose>
										<c:when test="${current > 1 }">
											<li><a href="${basePath }tutor?page=${current -1 }">上一页</a></li>
										</c:when>
										<c:otherwise>
											<li><span>« 上一页</span></li>
										</c:otherwise>
									...
									<c:forEach var="p" begin="1" end="${data.total / 15 +1}">
										<li><a href="${basePath }tutor?page=${p }">${p }</a></li>
									</c:forEach>
									<c:choose> 
										<c:when test="${current <= (data.total / 15)}"> 	
											<li><a href="${basePath }tutor?page=${current+1 }">下一页  »</a></li>
										</c:when>
										<c:otherwise>
											<li><a href="#">下一页 »</a></li>
										</c:otherwise>
									</c:choose>
								<li><a href="${basePath }tutor?page=<fmt:formatNumber type="number" value="${(data.total+1) / 15 }" maxFractionDigits="0"/>">末页</a></li>
								</ul>
							</nav>
						</div>
