											...
											<div class="featured-job-card-title">
												<h3>
													<a class="res-jobtitle"
														href="${basePath }job/repast/${item.id }" target="_blank">
														${item.title}</a>
												</h3>
											</div>
											<div class="featured-job-card-body">
												<div class="text-primary">类别：餐饮兼职</div>
							...
							<nav>
								...
								<c:choose>
										<c:when test="${current > 1 }">
											<li><a href="${basePath }repast?page=${current -1 }">上一页</a></li>
										</c:when>
										<c:otherwise>
											<li><span>« 上一页</span></li>
										</c:otherwise>
									...
									<c:forEach var="p" begin="1" end="${data.total / 15 +1}">
										<li><a href="${basePath }repast?page=${p }">${p }</a></li>
									</c:forEach>
									<c:choose> 
										<c:when test="${current <= (data.total / 15)}"> 	
											<li><a href="${basePath }repast?page=${current+1 }">下一页  »</a></li>
										</c:when>
										<c:otherwise>
											<li><a href="#">下一页 »</a></li>
										</c:otherwise>
									</c:choose>
									<li><a href="${basePath }repast?page=<fmt:formatNumber type="number" value="${(data.total+1) / 15 }" maxFractionDigits="0"/>">末页</a></li>
								</ul> 
							</nav> 
								...
								<div id="filter-work-type-list" class="sidebar-wrapper">
									<ul class="reset-list">
										<li class="filter-work-type-li"><a
											href="${basePath }repast" title="餐饮兼职">餐饮兼职</a></li>
										<li class="filter-work-type-li"><a
											href="${basePath }tutor" title="家教兼职">家教兼职</a></li>
										<li class="filter-work-type-li"><a
											href="${basePath }supermarket" title="超市兼职">超市兼职</a></li>
										<li class="filter-work-type-li"><a
											href="${basePath }flyer" title="传单兼职">传单兼职</a></li>
									</ul>
								</div>
							</div>
