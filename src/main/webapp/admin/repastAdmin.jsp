...
<script type="text/javascript">
	$(function() {
		$('#dg').datagrid({
			url : '${basePath}admin/getAllRepast',
			pageSize : 20
		})
	});
	...
	function destroyItem() {
		...
		if (row) {
			$.messager.confirm('确认', '是否确认删除该条目？', function(r) {
				if (r) {
					$.post('${basePath}admin/deleteRepast', {
						id : row.id
					}, function(result) {
	...
	function saveItem() {
		$('#fm').form('submit', {
			url : '${basePath}admin/saveOrEditRepast?' + Math.random(),
			onSubmit : function() {
...
<body>
	<table id="dg" title="餐饮兼职管理" class="easyui-datagrid" style=""
